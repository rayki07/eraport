        ...
        <form action="{{ route('kelas.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="rombel" id="rombel" required
                        ...
                        <option value="5" @if(old('rombel') == '5') selected @endif>5</option>
                        <option value="6" @if(old('rombel') == '6') selected @endif>6</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" required
                        ...
                        placeholder="Contoh: Tunisia" value="{{ old('nama') }}">
                    @error('nama') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                </div>
                
                {{-- Tombol Aksi --}}
                <div class="flex justify-end space-x-4 pt-4 border-t mt-6">
                    <a href="{{ route('kelas.index') }} " class="inline-flex items-center py-2 px-4 bg-gray-200 text-gray-700 font-semibold rounded-lg hover:bg-gray-300 transition-colors shadow-md">
