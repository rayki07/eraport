...
<div class="bg-white p-6 rounded-xl shadow-lg max-w-6xl mx-auto">
    ...
    <div class="border-b pb-4 mb-6">
                ...
                <div>
                    ...
                    <select name="siswa_id" id="siswa_id"
                        ...
                        @foreach ($siswa as $item)
                            <option
                                value="{{ $item->id }}"
                                {{-- Simpan data tambahan di atribut data --}}
                                data-nama="{{ $item->siswa->nama_lengkap }}"
                                data-nisn="{{ $item->siswa->nisn }}"
                                @if (old('siswa_id') == $item->id) selected   
                                @endif>
        ...
        <p id="info-nama" class="text-3xl font-extrabold text-red-600 mt-1"></p>
        <p id="info-detail" class="text-sm text-gray-500 mt-1"></p>
        <p class="text-sm text-gray-500 mt-1">Kelas: 3 RI | NISN: 0034</p>
    </div>
...
<script>
    ...
    function updateStudentInfo() {
        ...
        if (selectedOption.value) {
            ...
            // Perbarui teks di elemen P
            infoSiswa.textContent = 'Nama: ' + nama;
            infoDetail.textContent = 'NISN: ' + nisn;
        } else {
            // Kosongkan jika opsi "-- Pilih Siswa --" dipilih
            infoSiswa.textContent = '';
            infoDetail.textContent = '';
        }
    }

    // 2. Tambahkan event listener (jalankan fungsi saat dropdown berubah)
    selectSiswa.addEventListener('change', updateStudentInfo);

    // 3. Jalankan fungsi saat halaman dimuat (untuk menangani old() value)
    updateStudentInfo();
</script>
